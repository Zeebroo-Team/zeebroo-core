...
body.pos-walking-active .pos-online__body.pos-three-panel{padding-top:10px;}
...
<div class="pos-shell pos-online @if($posWalkingCustomer) pos-online--walking @endif {{ $posShellClass }}" data-pos-online>
    <header class="pos-online__top">
        ...
        <div class="pos-online__actions">
            <button type="button" class="pos-online__link" data-pos-add-product-open title="{{ __('Add product') }}" aria-label="{{ __('Add product') }}"><i class="fa fa-plus" aria-hidden="true"></i></button>
            @include('pos::partials.pos-settings-modal', [
                'posSettings'         => $posSettings,
                'accounts'            => $accounts,
            ...
            {{-- Walking-customer toggle posts to portal route (no business ownership required) --}}
            <form method="post" action="{{ route('hr.portal.pos-online.toggle-walking') }}" class="pos-walking-toggle">
                @csrf
                ...
                <label class="pos-walking-toggle__lbl" for="pos-walking-customer-switch">
                    ...
                    <span class="pos-walking-switch">
                        <input type="checkbox" id="pos-walking-customer-switch" name="enabled" value="1"
                            @checked($posWalkingCustomer) onchange="this.form.submit()">
    ...
    {{-- POS flash status intentionally hidden in POS flow --}}
    @if(session('status'))
        <div class="pos-online__banner pos-online__banner--ok" role="status">{{ session('status') }}</div>
    @endif
    @if($errors->any())
    ...
    @if(empty($products))
        <div class="pos-online__banner pos-online__banner--err" role="alert" data-pos-empty-products-banner>
            No products available. Ask your HR administrator to check the product catalog.
        </div>
    @endif
    ...
    <div class="pos-online__body pos-three-panel">
        ...
        <section class="pos-three-panel__center pos-online__catalog" aria-label="Product catalog">
            @if($categories->isNotEmpty())
                <nav class="pos-online__cats-bar pos-online__cats" aria-label="Categories">
                    <a href="{{ route('hr.portal.pos-online', $portalRouteParams()) }}" @class(['pos-online__cat', 'is-active' => !$categoryId])>All</a>
                    @foreach($categories as $category)
                        <a href="{{ route('hr.portal.pos-online', $portalRouteParams((int) $category->id)) }}" @class(['pos-online__cat', 'is-active' => (int) $categoryId === (int) $category->id])>{{ $category->name }}</a>
                    @endforeach
                </nav>
            @endif
        ...
        @if($checkoutModalEnabled)
            @include('pos::partials.pos-checkout-modal', [
                ...
                'channel'                => 'online',
                'discountFieldEnabled'   => $discountFieldEnabled,
                'checkoutFormAction'     => $checkoutFormAction,
            ])
        @else
            <section class="pos-three-panel__right pos-online__checkout" aria-label="Checkout">
                ...
                <div class="pos-online__checkout-body">
                    @include('pos::partials.pos-checkout-panel', [
                        'defaultDepositAccountId' => $defaultDepositAccountId,
                        'currency'               => $currency,
                        'channel'                => 'online',
                        'discountFieldEnabled'   => $discountFieldEnabled,
...
(function () {
    ...
    window.initPosAddProductModal?.({
        ...
        currencySuffix: currencySuffix,
        gridVariant: 'online',
        storeUrl: @json(route('hr.portal.pos-online.products.store')),
