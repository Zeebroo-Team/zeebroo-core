...
/* ── modal ── */
.arl-modal-overlay{position:fixed;inset:0;z-index:340;display:none;align-items:center;justify-content:center;padding:20px;box-sizing:border-box;}
.arl-modal-overlay.is-open{display:flex;}
.arl-modal-backdrop{position:absolute;inset:0;background:rgba(2,6,23,.55);backdrop-filter:blur(4px);cursor:pointer;}
:is(html[data-theme="light"],html[data-theme="light_blue"]) .arl-modal-backdrop{background:rgba(17,24,39,.42);}
.arl-modal-shell{position:relative;z-index:1;width:100%;max-width:500px;border-radius:18px;border:1px solid var(--border);
    background:var(--card);box-shadow:0 24px 56px rgba(0,0,0,.28);display:flex;flex-direction:column;max-height:calc(100vh - 40px);}
...
.arl-modal-body{padding:20px 22px;overflow-y:auto;flex:1;min-height:0;}
.arl-modal-foot{padding:14px 22px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;gap:10px;flex-shrink:0;}
...
<div class="arl-wrap">
  ...
  <div class="arl-header">
    ...
    <button class="arl-add-btn" onclick="document.getElementById('arl-modal').classList.add('is-open')">
...
<div class="arl-modal-overlay @if($errors->any()) is-open @endif" id="arl-modal">
  <div class="arl-modal-backdrop" onclick="document.getElementById('arl-modal').classList.remove('is-open')"></div>
  <div class="arl-modal-shell">
    <div class="arl-modal-head">
      <div>
        <h2 class="arl-modal-title">New Release</h2>
        <p class="arl-modal-sub">Publish a new desktop app version.</p>
      </div>
      <button class="arl-modal-close" type="button"
              onclick="document.getElementById('arl-modal').classList.remove('is-open')">
    ...
    <form method="POST" action="{{ route('admin.releases.store') }}">
      ...
      <div class="arl-modal-body">
        ...
        <div class="arl-field-row">
          <div class="arl-field">
            ...
            <input type="text" name="version" value="{{ old('version', '5.0.0') }}"
          ...
          <div class="arl-field">
            ...
            <input type="date" name="release_date" value="{{ old('release_date', now()->toDateString()) }}" required>
        ...
        <div class="arl-field">
          <label class="arl-check-row" style="text-transform:none;letter-spacing:0;font-weight:normal;cursor:pointer">
            <input type="checkbox" name="is_latest" value="1" {{ old('is_latest',1) ? 'checked' : '' }}>
        ...
        <div class="arl-field">
          ...
          <input type="url" name="windows_url" value="{{ old('windows_url') }}"
        ...
        <div class="arl-field">
          ...
          <input type="url" name="macos_url" value="{{ old('macos_url') }}"
        ...
        <div class="arl-field">
          ...
          <input type="url" name="linux_url" value="{{ old('linux_url') }}"
      ...
      <div class="arl-modal-foot">
        <button type="button" class="arl-cancel-btn"
                onclick="document.getElementById('arl-modal').classList.remove('is-open')">Cancel</button>
        <button type="submit" class="arl-submit-btn">
          <i class="fa fa-rocket"></i> Publish Release
        </button>
      </div>
    </form>
  </div>
