...
.po-doc-badge{display:inline-block;font-size:10px;font-weight:700;padding:3px 10px;border-radius:999px;background:color-mix(in srgb,{{ $accentColor }} 12%,white);border:1.5px solid {{ $accentColor }};color:{{ $accentColor }};margin-top:6px;letter-spacing:.04em;text-transform:uppercase;}
...
.pay-card{border-radius:10px;border:1px solid #e2e8f0;padding:12px 16px;background:#f8fafc;}
.pay-card__label{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#94a3b8;margin-bottom:4px;}
.pay-card__value{font-size:18px;font-weight:800;color:#1e293b;}
@media print{
...
<div class="print-bar no-print">
    <a href="{{ route('purchase.grn.show', $grn) }}" class="back-btn">&#8592; Back</a>
...
<div class="po-wrap">
    <div class="po-title-row">
        <div>
            <div class="po-title">{{ $grn->grn_number }}</div>
            @if($grn->purchase?->supplier)
                <div class="po-subtitle">Supplier: <strong>{{ $grn->purchase->supplier->name }}</strong></div>
            @endif
            <span class="po-doc-badge">Goods Receive Note</span>
        </div>
        <div style="text-align:right;">
            <div style="font-size:11px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:3px;">Received date</div>
            <div style="font-size:14px;font-weight:700;color:#1e293b;">{{ $grn->received_date->format('M j, Y') }}</div>
        </div>
    </div>

    <div class="po-meta-grid">
        <div class="po-meta-item">
            <div class="po-meta-item__label">GRN Number</div>
            <div class="po-meta-item__value">{{ $grn->grn_number }}</div>
        </div>
        <div class="po-meta-item">
            <div class="po-meta-item__label">Purchase Order</div>
            <div class="po-meta-item__value">{{ $grn->purchase?->po_number ?? '—' }}</div>
        </div>
        <div class="po-meta-item">
            <div class="po-meta-item__label">Supplier</div>
            <div class="po-meta-item__value">{{ $grn->purchase?->supplier?->name ?? '—' }}</div>
        </div>
        @if($grn->reference)
        <div class="po-meta-item">
            <div class="po-meta-item__label">Reference</div>
            <div class="po-meta-item__value">{{ $grn->reference }}</div>
        </div>
        @endif
        <div class="po-meta-item">
            <div class="po-meta-item__label">Currency</div>
            <div class="po-meta-item__value">{{ $currency ?: 'Default' }}</div>
        </div>
        <div class="po-meta-item">
            <div class="po-meta-item__label">Stock</div>
            <div class="po-meta-item__value">{{ $grn->stock_applied ? 'Updated' : 'Pending' }}</div>
        </div>
    </div>

    @if($grn->notes)
        <div class="po-notes">{{ $grn->notes }}</div>
    @endif
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Sell Price @if($currency)({{ $currency }})@endif</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ rtrim(rtrim(number_format((float)$item->quantity_received,3,'.','' ),'0'),'.') }} {{ $item->product?->productUnit?->name ?? '' }}</td>
                <td>{{ number_format((float)$item->unit_cost, 2) }}</td>
                <td>{{ $item->selling_unit_price !== null ? number_format((float)$item->selling_unit_price,2) : '—' }}</td>
        ...
        <tfoot>
            <tr>
                ...
                <td>{{ number_format((float)$grnTotal, 2) }}@if($currency) {{ $currency }}@endif</td>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="pay-summary">
        <div class="pay-card">
            <div class="pay-card__label">GRN Total</div>
            <div class="pay-card__value">{{ number_format($grnTotal,2) }}@if($currency) <span style="font-size:12px;font-weight:600;color:#64748b;">{{ $currency }}</span>@endif</div>
        </div>
        <div class="pay-card">
            <div class="pay-card__label">Paid</div>
            <div class="pay-card__value" style="color:#16a34a;">{{ number_format($amountPaid,2) }}@if($currency) <span style="font-size:12px;font-weight:600;color:#64748b;">{{ $currency }}</span>@endif</div>
        </div>
        <div class="pay-card">
            <div class="pay-card__label">Outstanding</div>
            <div class="pay-card__value" style="color:{{ $amountOutstanding > 0.005 ? '#f59e0b' : '#94a3b8' }};">{{ number_format($amountOutstanding,2) }}@if($currency) <span style="font-size:12px;font-weight:600;color:#64748b;">{{ $currency }}</span>@endif</div>
        </div>
    </div>

    <div style="display:flex;justify-content:flex-end;margin-top:20px;gap:60px;">
