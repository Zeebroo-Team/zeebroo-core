...
<div class="pcat-page-card card" style="max-width:100%;padding:14px;">
    ...
    <p class="muted" style="margin:0 0 14px;font-size:13px;line-height:1.45;">
        All processed returns for <strong style="color:var(--text);">{{ $business->name }}</strong>. Each return restores stock and records the refund method.
    </p>
    ...
    <div class="pcat-toolbar">
        @if($hasReturns)
            <form method="get" action="{{ route('pos.returns.index') }}" style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;">
        ...
        @endif
        <a href="{{ route('pos.returns.create') }}" class="linkbtn" style="padding:8px 16px;font-size:13px;display:inline-flex;align-items:center;gap:6px;margin-left:auto;">
    ...
    @if(!$hasReturns)
        <section class="pcat-inline" style="margin-top:8px;">
            ...
            <p class="pcat-muted">Use the <strong>Create return note</strong> button to look up a sale and process a return. Stock is restored automatically and the refund method is recorded.</p>
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;">
                <a href="{{ route('pos.returns.create') }}" class="linkbtn" style="display:inline-flex;align-items:center;gap:6px;">
                    <i class="fa fa-rotate-left"></i> Create return note
                </a>
                <a href="{{ route('pos.sales.index') }}" class="pcat-link" style="display:inline-flex;align-items:center;gap:6px;align-self:center;">
        ...
        <div class="pcat-table-wrap">
            <table class="pcat-table">
                ...
                <tbody>
                    @forelse($returns as $ret)
                        <tr>
                            ...
                            <td style="text-align:right;">
                                @if($ret->sale)
                                    <a href="{{ route('pos.sales.show', $ret->sale) }}#returns" class="pcat-link" style="font-weight:700;">View sale</a>
                                @endif
                            </td>
                        </tr>
                        {{-- Expanded item rows --}}
                        ...
                        @if(filled($ret->notes))
                            <tr style="background:color-mix(in srgb,var(--border) 8%,transparent);">
                                <td></td>
                                <td colspan="8" style="padding-left:20px;font-size:12px;color:var(--muted);padding-top:4px;padding-bottom:8px;">
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="muted" style="padding:16px;">No returns match your search.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
