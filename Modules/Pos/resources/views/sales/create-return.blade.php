...
<div class="pcat-page-card card" style="max-width:880px;padding:14px;">
    ...
    {{-- ── Mode toggle ──────────────────────────────────────────── --}}
    <div class="crn-mode">
        <a href="{{ route('pos.returns.create') }}" class="crn-mode__btn {{ $mode === 'ref' ? 'crn-mode__btn--active' : '' }}">
            <i class="fa fa-receipt"></i> With sale reference
        </a>
        <a href="{{ route('pos.returns.create', ['mode' => 'open']) }}" class="crn-mode__btn {{ $mode === 'open' ? 'crn-mode__btn--active' : '' }}">
    ...
    @if($mode === 'ref')
        ...
        @if($sale)
            <div class="crn-sale-info">
                ...
                <a href="{{ route('pos.sales.show', $sale) }}" class="pcat-link" style="font-size:12px;margin-left:auto;">View sale <i class="fa fa-arrow-up-right-from-square"></i></a>
            </div>
        ...
        @if(!filled($saleNumber))
            <div class="crn-alert crn-alert--info" style="margin-top:4px;">
                <i class="fa fa-circle-info" style="flex-shrink:0;margin-top:2px;"></i>
                <span>Enter a sale number above to load items, or switch to <a href="{{ route('pos.returns.create', ['mode' => 'open']) }}" class="pcat-link">without sale reference</a> to return products directly.</span>
            </div>
        @endif
    ...
    @else
        ...
        <form method="post" action="{{ route('pos.returns.store-open') }}" id="crnOpenForm">
            ...
            <div class="pcat-table-wrap" style="margin-bottom:0;">
                <table class="crn-table" id="crnOpenTable">
                    ...
                    <tbody id="crnOpenBody">
                        {{-- rows added by JS; show one blank row to start --}}
                    </tbody>
                </table>
            </div>
        ...
        (function () {
            ...
            // Product lookup by search string
            ...
            // Delegate qty/price input → recalc
