@extends('theme::layouts.app', ['title' => 'Reporters', 'heading' => 'Reporters'])
...
<div class="pcat-page-card card" style="max-width:100%;padding:14px;">
    ...
    <div class="bmg-toolbar">
        <form method="get" action="{{ route('pos.brand-mgmt.reporters.index') }}" class="bmg-search">
            ...
            @if($search)
                <a href="{{ route('pos.brand-mgmt.reporters.index') }}" class="linkbtn" style="padding:8px 12px;font-size:13px;background:transparent;border:1px solid var(--border);color:var(--text);text-decoration:none;">Clear</a>
            @endif
        </form>
        <button type="button" class="linkbtn" id="bmg-add-btn" style="padding:8px 16px;font-size:13px;display:inline-flex;align-items:center;gap:6px;">
            <i class="fa fa-plus"></i> Add reporter
        </button>
    </div>
    ...
    <div class="bmg-table-wrap">
        @if($reporters->isEmpty())
            <div class="bmg-empty">
                ...
                <p>{{ $search ? 'No reporters matched "'.e($search).'".' : 'No reporters yet. Add your first reporter to get started.' }}</p>
            </div>
        @else
        <table class="bmg-table">
            ...
            <tbody>
                ...
                <tr>
                    <td><strong style="color:var(--text);">{{ $reporter->name }}</strong></td>
                    <td style="font-size:12px;">{{ $reporter->email }}</td>
                    <td style="text-align:right;">
                        <div class="bmg-actions">
                            <button type="button" class="bmg-action-btn bmg-action-btn--edit"
                                data-edit
                                data-id="{{ $reporter->id }}"
                                data-name="{{ e($reporter->name) }}"
                                data-email="{{ e($reporter->email) }}"
                            ><i class="fa fa-pen-to-square"></i> Edit</button>
                            <form method="post" action="{{ route('pos.brand-mgmt.reporters.destroy', $reporter) }}" onsubmit="return confirm('Delete {{ addslashes($reporter->name) }}?');" style="margin:0;">
                                @csrf @method('DELETE')
    ...
    <p class="muted" style="margin:10px 0 0;font-size:12px;">
        {{ $reporters->count() }} reporter{{ $reporters->count() === 1 ? '' : 's' }}{{ $search ? ' matching "'.e($search).'"' : '' }}
    </p>
</div>
...
<div id="bmg-modal" class="bmg-modal" role="dialog" aria-modal="true" aria-labelledby="bmg-modal-title" aria-hidden="true">
    ...
    <div class="bmg-modal__panel">
        <div class="bmg-modal__head">
            <h3 id="bmg-modal-title">Add reporter</h3>
            <button type="button" class="bmg-modal__close" id="bmg-modal-close" aria-label="Close"><i class="fa fa-times"></i></button>
        </div>
        <form id="bmg-form" method="post" action="{{ route('pos.brand-mgmt.reporters.store') }}">
            @csrf
            <input type="hidden" name="_method" id="bmg-form-method" value="POST">
            <div class="bmg-modal__body">
                <p class="muted" style="font-size:12px;margin:0;">Adding a reporter also creates a portal login they can use to sign in.</p>
                <div class="bmg-field">
            ...
            <div class="bmg-modal__foot">
                ...
                <button type="submit" class="bmg-btn-primary"><i class="fa fa-floppy-disk"></i> Save reporter</button>
            </div>
        </form>
    </div>
</div>
...
(function () {
    ...
    var baseAction = @json(route('pos.brand-mgmt.reporters.store'));
    var updateBase = @json(url('/pos/brand-mgmt/reporters'));
    ...
    function closeModal() {
        ...
        methodEl.value = 'POST';
        titleEl.textContent = 'Add reporter';
    ...
    document.querySelectorAll('[data-edit]').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            methodEl.value = 'PUT';
            titleEl.textContent = 'Edit reporter';
