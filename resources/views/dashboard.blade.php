...
@php
    ...
    // Home tab content (KPIs, Business Flow, Today, Recent Activity, Analytics,
    // Expenses, Profit, Payroll, Orders, CRM) now lives inline on this Overview
    // page rather than behind a separate /home route.
    ...
    $homeOverview = ($business && $hasBankAccountForBusiness)
        ? app(\App\Services\HomeOverviewService::class)->forBusiness($business)
        : null;
...
@else
    ...
    @php extract($homeOverview); @endphp
    <style>
    ...
    <div class="home-subnav">
        ...
        <button type="button" class="home-tab-btn" data-home-view="quicksetup"><i class="fa fa-layer-group"></i> Quick Setup</button>
    ...
    <div class="home-body">
        <div class="home-main">
                            ...
                            <div class="hog-grid">
                                @foreach($section['cards'] as $card)
                                    @php
                                        ...
                                        $href = $isHomeSelf ? '#' : (Route::has($card['route']) ? route($card['route']) : '#');
                                    @endphp
                                    <a href="{{ $href }}" @if($isHomeSelf) data-home-jump="{{ collect(['Today\'s Summary'=>'today','Recent Activity'=>'activity','Profit Report'=>'profit','Analytics'=>'analytics'])[$card['label']] ?? 'overview' }}" @endif class="hog-card">
            ...
            {{-- ── Quick Setup ── --}}
            <div id="home-view-quicksetup" class="home-view">
    ...
    (function () {
        ...
        window.addEventListener('resize', function () {
            var flowView = document.getElementById('home-view-flow');
            if (flowView && flowView.classList.contains('active')) fit();
        });
